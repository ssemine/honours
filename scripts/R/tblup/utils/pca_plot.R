...
library(ggrepel)  # better for overlapping labels
library(dplyr)
# Filter points to label
labels_df <- scores2 %>%
  filter(orm_mean > 0.35 | PC1 < -100)

# Original plot with conditional labels
p <- ggplot(scores2, aes(x = PC1, y = PC2, color = orm_mean)) +
  ...
  geom_text_repel(
    ...
    bg.color = "black",   # background behind text
    bg.r = 0.15,          # roundness of background box
    box.padding = 0.3,
    point.padding = 0.3,
    max.overlaps = Inf
  ) +
